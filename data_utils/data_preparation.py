...
max_document_length = 999  # 每一条犯罪事实最大的长度
...
# 取出law和accu数据
# relevant articles
label_type = 'articles'  # 处理的标签
train_data_process.decompose_data('articles','./train.csv')  # decompose data - 取出原始数据中的每一部分数据, 主要为law/fact/accusation/
train_law_label = train_data_process.decomposition['articles']
train_data_process.get_label_collection('articles')
train_law = train_data_process.transform_label(train_law_label, label_type='articles')
...
train_data_process.decompose_data('accusation','./train.csv')  # decompose data - 取出原始数据中的每一部分数据, 主要为law/fact/accusation/
train_accu_label = train_data_process.decomposition['accusation']
train_data_process.get_label_collection(label_type='accusation')
train_accu = train_data_process.transform_label(labels=train_accu_label, label_type='accusation')
...
#test_data_process.get_data('./test.csv')

# 把test data中的text取出
test_data_process.decompose_data('fact','./test.csv')
data_test_text = test_data_process.decomposition['fact']

#data_test_text=data_test_text[0:10000]
...
# relevant_articles
label_type = 'articles'  # 处理的标签
test_data_process.decompose_data('articles','./test.csv')
test_law_label = test_data_process.decomposition['articles']
test_data_process.get_label_collection(label_type='articles')
test_law = test_data_process.transform_label(labels=test_law_label, label_type='articles')
...
# accusation
label_type = 'accusation'  # 处理的标签
test_data_process.decompose_data('accusation','./test.csv')  # decompose data - 取出原始数据中的每一部分数据, 主要为law/fact/accusation/
test_accu_label = test_data_process.decomposition['accusation']
test_data_process.get_label_collection(label_type='accusation')
test_accu = test_data_process.transform_label(labels=test_accu_label, label_type='accusation')
...
# 把valid data中的fact取出
valid_data_process.decompose_data('fact','./valid.csv')
data_valid_text = valid_data_process.decomposition['fact']

#data_valid_text=data_valid_text[0:20000]
...
# relevant_articles
label_type = 'articles'  # 处理的标签
valid_data_process.decompose_data('articles','./valid.csv')
valid_law_label = valid_data_process.decomposition['articles']
valid_data_process.get_label_collection(label_type='articles')
valid_one_hot = valid_data_process.transform_label(labels=valid_law_label, label_type='articles')
...
# accusation
label_type = 'accusation'  # 处理的标签
valid_data_process.decompose_data('accusation','./valid.csv')  # decompose data - 取出原始数据中的每一部分数据, 主要为law/fact/accusation/
valid_accu_label = valid_data_process.decomposition['accusation']
valid_data_process.get_label_collection(label_type='accusation')
valid_accu = valid_data_process.transform_label(labels=valid_accu_label, label_type='accusation')
